...
const Carousel = ({ items, title }) => { // TODO: dummy data 삭제
    ...
    const { totalPages, isNextDisabled, isPrevDisabled, displayedItems, emptySpaces } = useMemo(() => {
        ...
        const displayedItems = safeItems.slice(currentIndex * ITEMS_PER_PAGE, (currentIndex + 1) * ITEMS_PER_PAGE);
        return {
            totalPages,
            isNextDisabled: currentIndex >= totalPages - 1,
            isPrevDisabled: currentIndex <= 0,
            displayedItems,
            emptySpaces: ITEMS_PER_PAGE - displayedItems.length
        };
    }, [items, currentIndex]);
                ...
                <div className="carousel__content">
                    ...
                    <ul className="carousel__list">
                        ...
                        {displayedItems.map((item, index) => (
                            <CarouselItem key={`item-${currentIndex * ITEMS_PER_PAGE + index}`} text={item.text} />
                        ))}
...
const NavButton = ({ direction, onClick, disabled }) => (
    <div className={`carousel__nav-btn carousel__nav-btn--${direction} ${disabled ? 'carousel__nav-btn--disabled' : ''}`} aria-label={`${direction === 'prev' ? '이전' : '다음'} 항목`}>
