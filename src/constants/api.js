...
export const FEED_ROOT_API_URL = (feedType, category) =>
  `${feedType}s/${feedType === FEED_TYPE.NOTICE && category === NOTICE_TYPE.CAMPUS ? 'all' : category}`;
...
export const IMAGE_API_URL = image => `${process.env.REACT_APP_API_BASE_URL}view/${image}`;
export const THUMBNAIL_API_URL = image => `${process.env.REACT_APP_API_BASE_URL}view/s_${image}`;
